...
class VertexRemoverEdgeCasesTest {
    ...
    void testRepeatedVertexIndicesInList() {
        Model model = new Model();
        model.getVertices().addAll(Arrays.asList(
                new Vector3f(0, 0, 0),  // вершина 1
                new Vector3f(1, 0, 0),  // вершина 2
                new Vector3f(0, 1, 0)   // вершина 3
        ));
        ...
        assertEquals(2, model.getVertices().size(),
                "Дублирующиеся индексы должны игнорироваться - удалена только одна вершина");
        assertEquals(0, model.getPolygons().size(),
                "Полигон, содержащий удаляемую вершину, должен быть удален");
    }
}